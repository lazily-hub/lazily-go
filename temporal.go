...
func NewTimerCell(ctx *Context, fireAt uint64) *TimerCell {
	return &TimerCell{core: NewTimerCore(fireAt), fired: NewCell(ctx, false)}
}
...
func (t *TimerCell) FiredCell() *Cell[bool] { return t.fired }
...
func NewIntervalCell(ctx *Context, period uint64) *IntervalCell {
	return &IntervalCell{core: NewIntervalCore(period), count: NewCell[uint64](ctx, 0)}
}
...
func (iv *IntervalCell) CountCell() *Cell[uint64] { return iv.count }
...
func NewCronCell(ctx *Context, cycle uint64, offsets []uint64) *CronCell {
	return &CronCell{core: NewCronCore(cycle, offsets), count: NewCell[uint64](ctx, 0)}
}
...
// CountCell returns the backing count cell.
func (c *CronCell) CountCell() *Cell[uint64] { return c.count }
...
func (d *DeadlineCell[T]) ExpiredCell() *Cell[bool] { return d.expired }
